...
import { buildTree } from '@/lib/folderUtils';
...
export default function FolderTree() {
  const { folders, isLoading, error, fetchFolders, reorderFolders } =
    useFolderStore();
  ...
  const [dragDirection, setDragDirection] = useState<'above' | 'below' | null>(
    null,
  );
  const {
    isOpen: isCreateOpen,
  ...
  const dragOverlayBorderColor = useColorModeValue('gray.200', 'gray.600');
        ...
        <DragOverlay>
          {activeId ? (
            <Box
              ...
              border="1px solid"
              borderColor={dragOverlayBorderColor}
            >
