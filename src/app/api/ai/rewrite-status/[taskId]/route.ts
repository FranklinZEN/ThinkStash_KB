...
export async function GET(
  ...
  try {
    ...
    console.info('Task fetched successfully', {
      ...
      status: task.status,
      userId: task.userId,
      progressStage: task.progressMessage,
      hasResultData: task.result != null,
    });
    ...
    if (task.status === 'COMPLETED') {
      if (task.result) {
        try {
          ...
          const resultDataObject = JSON.parse(task.result as string);
          ...
          console.error('Task COMPLETED but failed to parse resultData JSON.', {
            ...
            error:
              parseError instanceof Error
                ? parseError.message
                : String(parseError),
            resultDataPreview: (task.result as string).substring(0, 100),
          });
        }
      } else {
      ...
      responsePayload.errorMessage = task.error as string; // Assuming error is a string
