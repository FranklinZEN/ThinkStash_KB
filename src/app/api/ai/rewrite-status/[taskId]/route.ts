...
export async function GET(
  ...
  try {
    ...
    if (task.status === 'COMPLETED') {
        if (task.resultData) {
            try {
                ...
                if (resultDataObject && resultDataObject.ai_rewritten_content_blocks) {
                    responsePayload.ai_rewritten_content_blocks = resultDataObject.ai_rewritten_content_blocks;
                } else {
                    console.warn("Task COMPLETED and resultData parsed, but 'ai_rewritten_content_blocks' key is missing or null.", {
                        taskId: resolvedTaskId,
                    });
                }
            } catch (parseError) {
    ...
    console.info("Returning task status successfully.", { taskId: resolvedTaskId, status: responsePayload.status });
    return NextResponse.json(responsePayload);

  } catch (error) {
    console.error("Unhandled error in /ai/rewrite-status/[taskId] API route", {
      taskId: resolvedTaskId || (params as any)?.taskId || "unknown_taskId_in_catch",
