...
class ImageProcessingService(BaseService):
    ...
    async def _upload_to_gcs(self, image_bytes: bytes, gcs_blob_name: str, mime_type: Optional[str]) -> str:
        if not self.gcs_client or not self.gcs_bucket_name:
            raise ValueError("GCS client or bucket not configured.")
        ...
        try:
            async with self.gcs_upload_semaphore:
                ...
                # This allows temporary, secure access for authenticated users
                # without making the bucket public.
                # Note: The service account needs the "Service Account Token Creator" IAM role.
                ...
                self.logger.info(f"ImageProcessingService: Successfully uploaded and generated signed URL for {gcs_blob_name}")
                return signed_url
        except Exception as e:
            self.logger.error(f"ImageProcessingService: ERROR - GCS Upload failed for {gcs_blob_name}: {str(e)}", exc_info=True)
            raise IOError(f"GCS upload failed: {str(e)}") from e
    ...
    async def _process_single_image(self, raw_image: RawImageInput) -> Optional[EnrichedImageMetadata]:
        ...
        if self.gcs_client and self.gcs_bucket_name:
            ...
            try:
                gcs_url = await self._upload_to_gcs(current_image_bytes, gcs_blob_name, metadata.get("mime_type"))
            except IOError as e:
                error_messages.append(str(e))
                self.logger.error(f"ImageProcessingService: GCS upload failed for {raw_image.image_id}: {str(e)}", exc_info=False)
